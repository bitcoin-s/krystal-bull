...
class SeedStorageTest extends BitcoinSFixture {
  ...
  override type FixtureParam = KrystalBullAppConfig

  def tmpDir(): Path = Files.createTempDirectory("krystal-bull-")

  override def withFixture(test: OneArgAsyncTest): FutureOutcome = {
    val builder: () => Future[KrystalBullAppConfig] = () => {
  ...
  def getSeedPath(config: KrystalBullAppConfig): Path = {
    config.seedPath
  }
  ...
  def getAndWriteMnemonic(conf: KrystalBullAppConfig): DecryptedMnemonic = {
  ...
  it must "write and read a mnemonic to disk" in { conf: KrystalBullAppConfig =>
